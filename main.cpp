...
public:
    ...
    void generateRandomInstance() {
        ...
        for (int j = 0; j < T; j++) {
            // Each target aims at one asset (simplified)
            int assetIndex = j % K;
        ...
        // Generate engagement feasibility
        for (int t = 0; t < S; t++) {
            double ratio = 0.1 + 0.8 * ((double)t / (S - 1));
            for (int i = 0; i < W; i++) {
                for (int j = 0; j < T; j++) {
        ...
        for (int i = 0; i < W; i++) {
            maxWeaponUses[i] = 1 + (rand() % S);  // Each weapon can be used 1 to S times
        }
    }
...
int main() {
    ...
    int W = 100;  // Number of weapons
    int T = 100;  // Number of targets
